test_that("saved_model expects udpipe_model class", {
  ...
  model_loaded <- limpiar_pos_load_model("data/french-gsd-ud-2.5-191206.udpipe")
  output <- limpiar_pos_annotate(data = data, text_var = text, saved_model = model_loaded, in_parallel = TRUE, parse_text = FALSE)
...
test_that("in_parallel speeds up processing time", {
  ...
  output <- limpiar_pos_annotate(data = data, text_var = text, saved_model = model_loaded, in_parallel = FALSE, parse_text = FALSE)
...
test_that("dependency parsing works as intended", {
  ...
  output <- limpiar_pos_annotate(data = data, text_var = text, saved_model = model_loaded, in_parallel = FALSE, parse_text = FALSE)
  expect_true(all(is.na(output$dep_rel)))

  output_w_parsing <- limpiar_pos_annotate(data = data, text_var = text, saved_model = model_loaded, in_parallel = FALSE, parse_text = TRUE)
  expect_false(any(is.na(output_w_parsing$dep_rel)))
})
