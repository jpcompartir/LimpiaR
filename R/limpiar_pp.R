...
limpiar_pp_products <- function(df, text_var){
  ...
  entities <- entities %>%
    dplyr::filter(!token %in% c("\\bmicrosoft\\b","\\b|apple\\b", "\\bsamsung\\b", "\\bwhatsapp\\b", "\\bintel\\b", "\\bamazon\\b", "\\bsony\\b", "\\binstagram\\b"))
...
limpiar_pp_companies <- function(df, text_var){
  companies <- c("\\bapple\\b", "\\bmicrosoft\\b", "\\bmsft\\b", "\\bnvidia\\b", "\\bsony\\b", "\\binstagram\\b",
                 "\\bwhatsapp\\b", "duckduckgo", "\\bddg\\b", "\\bduck duck go\\b")
  replacement <- "company"

  df %>%
    dplyr::mutate({{text_var}} := stringr::str_replace_all({{text_var}}, companies))
}
