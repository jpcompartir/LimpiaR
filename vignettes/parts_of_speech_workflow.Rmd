...
data <- dplyr::tibble(text = tolower(stringr::sentences[1:100]), document = 1:100) %>% 
  dplyr::mutate(universal_message_id = paste0("TWITTER", sample(1:100, 100, replace = FALSE)))
```
...
```{r} 
annotations <- limpiar_pos_annotate(data = data, text_var = text, id_var = universal_message_id, pos_model = model, in_parallel = FALSE, parse_text = TRUE)
```
...
annotations %>% 
  ...
  DT::datatable()
```
So here, we can see that 'blue' along with 'clear' and 'third' are the most used adjectives in our sentence data. Now lets look at frequent nouns.
```{r}
annotations %>% 
  ...
  DT::datatable()
```
Okay, so we can see that 'fence' and 'week' are the most used nouns within the data set. With that sort of information we can begin to think about Nouns and Adjectives and their likelihood to occur in any one data set together.
