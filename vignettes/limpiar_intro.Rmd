...
limpiar_inspect(df, pattern = "ntonces", text_var = mention_content,
                post_url = mention_url, title = "ntonces")
```
...
## Bonus Function
Once you're familiar with how LimpiaR works, you may want to use a bonus function, limpiar_df to do a bunch of things at once and speed everything up. limpiar_df also has an option to remove punctuation!

```{r}
limpiar_df(df, text_var = mention_content, remove_punctuation = TRUE)
```

If you wanted to, you could pipe in limpiar_na_cols, limpiar_emojis (or any other pipe-able function!).
